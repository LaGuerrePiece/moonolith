...
watch(
	() => props.tool,
	(code) => {
		console.log('PRENIER TOOL', props.tool, 'CODE', code);
		if (code === Tool.DONE) {
			// stopUsingTool()
...
watch(
	...
	(good) => {
		console.log('props.hasBought.value', props.hasBought.value);
		if (props.hasBought.value === 1) {
			preEncode(grid).then((res) => {
				chunkCreator(res);
			});
		}
		console.log('props.hasBought AVANT', props.hasBought.value);
...
function useTool() {
	...
	if (newMousePosition.x === oldMousePosition.x && newMousePosition.y === oldMousePosition.y)
		return;
	...
	switch (props.tool) {
		case Tool.PEN:
			grid.draw_pixel(
				newMousePosition.x,
				newMousePosition.y,
				new Klon(props.color, Klon.PAINTED)
			);
			break;
		case Tool.ERASER:
			...
			for (let x = 0; x < width; x++) {
				...
				if (array[idx + 3] != 0)
					grid.draw_pixel(
						...
						y + offsety,
						new Klon(
							[array[idx] / 255, array[idx + 1] / 255, array[idx + 2] / 255],
							Klon.PAID
						)
					);
			}
